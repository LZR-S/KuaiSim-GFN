...
ENV_CLASS='KRCrossSessionEnvironmentCumulativeInfluence'
MAX_SESSION=6
MAX_RET_DAY=10
MAX_STEP=5
RET_DAY_BIAS=0.4
PERSONALIZATION_INF_RETURN=0.1
FEEDBACK_INF_RETURN=0.4
PAST_DECAY=0.7
# 0.3 1.0
...
# critic arguments
CRITIC1_CLASS='VCritic'

CRITIC2_CLASS='BCritic'
...
do
    ...
    do
        for CRITIC_LR in 0.0001 # 0.0001
        do
            for ACTOR_LR in 0.00002 # 0.0003 # 0.00001
            do
                for SEED in 11 # 11 13 17 19 23
                do
                    file_key=gfn4retention_PAUR_${POLICY_CLASS}_${ENV_CLASS}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_fret${FEEDBACK_INF_RETURN}_seed${SEED}
                    
                    mkdir -p ${output_path}/${file_key}/

                    python train_gfn_PAUR.py\
                        --seed ${SEED}\
                        --cuda 1\
                        --env_class ${ENV_CLASS}\
                        ...
                        --personalized_return_coef ${PERSONALIZATION_INF_RETURN}\
                        --past_reward_decay ${PAST_DECAY}\
                        ...
                        --critic1_class ${CRITIC1_CLASS}\
                        --critic2_class ${CRITIC2_CLASS}\
                        --critic1_hidden_dims 128\
                        --critic2_hidden_dims 32\
                        --critic1_dropout_rate 0.1\
                        --critic2_dropout_rate 0.1\
